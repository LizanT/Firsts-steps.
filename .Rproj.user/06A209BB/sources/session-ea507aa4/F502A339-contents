...
df_progress <- df_performance %>%
  ...
  t() %>%                              # And I use transpose matrix "t()" because "apply()" changed rows to columns and I need to reverse that change.
  as.data.frame()                      # Then, this matrix need to be a DataFrame, for that as.data.frame() funciton.
...
most_improved <- athlete_stats %>%
  arrange(desc(Absolute_Improvement)) %>%
  slice(1)

# Which athlete is most consistent?
most_consistent <- df_consistency %>%
  arrange(Coefficient_Variation) %>%
  slice(1)
